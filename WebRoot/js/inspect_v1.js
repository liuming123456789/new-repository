            ...
            */
            ...
            function getRequest() {
                ...
                request = 'idsite=' + configTrackerSiteId +
                    ...
                    '&res=' + screenAlias.width + 'x' + screenAlias.height + '&col=' + screen.colorDepth + "-bit" +
...
var pkBaseURL = (("https:" == document.location.protocol) ? "https://eop.mall.10010.com/alay/" : "http://eop.mall.10010.com/alay/");
 ...
 * mcId 营销活动编码
 * adId 广告位编码
 * clickId 点击事件:onClick事件里填1，否则填0
 * clickUrl 素材url
 * currentUrl 素材关联页面url
 */
// function recordLog(mobile, channelId, mcId, adId, clickId, clickUrl, currentUrl) {
//     try {
//         var piwikTracker;
//         if ("1" == clickId) {
//             piwikTracker = Piwik.getTracker(pkBaseURL + "app?service=page/Analytic&listener=getCustInfo&mobile=" + mobile + "&mc_id=" + mcId + "&ad_id=" + adId + "&channel_id=" + channelId + "&click_id=" + clickId + "&joinUrl=" + currentUrl + "&", 1);
//             piwikTracker.setCustomUrl(clickUrl);
//         }
//         else {
//             piwikTracker = Piwik.getTracker(pkBaseURL + "app?service=page/Analytic&listener=getCustInfo&mobile=" + mobile + "&mc_id=" + mcId + "&ad_id=" + adId + "&channel_id=" + channelId + "&click_id=" + clickId + "&", 1);
//         }
//         piwikTracker.trackPageView();
//         piwikTracker.enableLinkTracking();
//     } catch (err) {
...
function recordLogs() {
    try {
        var piwikTracker;
        // piwikTracker = Piwik.getTracker(pkBaseURL + "app?service=page/Analytic&listener=getCustInfo&mobile=&", 1);
        piwikTracker = Piwik.getTracker("/alay/app?service=page/Analytic&listener=getCustInfo&mobile=&", 1);
...
function ready(fn) {
    if (window.location.href.indexOf('inspect=0') > 0) {
        return;
    }
